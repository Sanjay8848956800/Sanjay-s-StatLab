...
# 1 Calculate correlation coefficient of husband and wives

$\begin{array}{|l|cccccccccc|}\hline
\text{Age of husband)}:& 23& 27& 28& 29& 30& 31& 33& 35& 36& 39 \\\hline
\text{Age of WIFE)}:& 18& 22& 23& 24& 25&  26& 28& 29& 30& 32 \\\hline
\end{array}$

## Rcode
```{r}
husband = c(23,27,28,29,30,31,33,35,36,39) 
wife = c(18,22,23,24,25,26,28,29,30,32)
```

```{r}
resp1=cor.test(husband,wife,method='pearson')
resp1
```
## Result and Discussion 
Since there is a correlation coefficient greater than 0.
5 and close to 1, there is a highly positive correlation coffefient.

#2 Rank of same 156 students in two subjects Statistics and accountancy are given below , the 2 numbers within the brackets denoting the ranks of the same students in statistics and accountancy respectively.Spearman's formula.(1,10), (2,7), (3,2), (4,6), (5,4), (6,8), (7,3), (8,1), (9,11), (10,15), (11,9), (12,5), (13,14), (14,12), (15,13). Use Spearman’s formula to find the rank correlation coefficient.
...
resp1=cor.test(S,A,method='spearman')
resp1
```

## Result and conclusion

Since it is close to 0.5 it is positively correlated only
...
marketing=data.frame(sales=c(91,97,108,121,67,124,51,73,111,57),purchase=c(71,75,69,97,70,91,39,61,80,47))
```
...
```{r}
library(ggplot2)
ggplot(marketing, aes(x = purchase, y = sales)) +
  geom_point() +
  stat_smooth()
```
...
```{r}
cor.test(marketing$sales,marketing$purchase)
```
### Create the OLS model

```{r}
#fit a linear model                                                                                                                                                                               
model <- lm(sales ~ purchase, data = marketing)
...
```{r, message=FALSE}
library(dplyr)
newdata <- data.frame(purchase = c(1000,  2000, 3500))
model %>% predict(newdata)
```
## Result & Discussion
Followed data obtained regration 

## 4Calculate the value of the Karl Pearson’s Correlation Coefficient.

$\begin{array}{|l|c|c|c|c|c|c|c|c|c|}\hline
\text{X}:& 2& 2& 3& 4& 4& 5& 6& 6& 7& 7&\\\hline
\text{Y}:& 8& 13& 14& 11& 20& 10& 22& 26& 22& 25&\\\hline
\end{array}$
...
```{r}
resp1=cor.test(x,y,method='pearson')
resp1
```
```{r}
#direct calculation
...
# 5 Calculate Spearman’s rank correlation coefficient between advertisement cost and sales from the following data

$\begin{array}{|l|l|l|l|l|l|l|l|l|l|l|}
\hline
\text{Advertisement Cost ('000 Rs)}:&39 &65 &62 &90 &82 &75 &25 &98 &36 &78\\\hline
\text{Sales}:&47 &53 &58 &86 &62 &68 &60 &91 &51 &84\\\hline
\end{array}$

```{r}
#loading data
Advertisement=c(39, 65, 62, 90, 82, 75, 25, 98, 36, 78)
...
```{r}
res3=cor.test(Advertisement,Sales,method='spearman')
res3
```
...
The Spearman's rank correlation coefficient of Advertisement and Sales  is 0.8181818
Since the correlation coefficient is greater than 0.5 it is highly positively correlated.

# 6 From the data given below, find the two regression equations
...
# 7 A die was thrown 9000 times and of these 3220 yielded a 3 or 4. Can the die be regarded as unbiased?
...
#8 In a random sample of 400 persons from a large population, 120 are females. Can it be said that males and females are in the ratio 5:3 in the population? Use 1% significance?
...
# 9 The adjoining table shows the number of motor registrations in a certain territory for a term of 5 years and the sale of motor tyres by a firm in that territory for the same period. Find the regression equation to estimate the sale of Tyres when motor registration is known. Estimate sale of Tyres when registration is 850.
...
# 10 In a big city 325 men out of 600 men were found to be smokers. Does this information support the conclusion that the majority of men in this city are smokers? Justify your answer statistically.
...
# 12 Certain pesticide is packed into bags by a machine. A random sample of 10 bags drawn and their contents are found to weigh (in kg.) as follows:
...
# 13 In two large populations there are 30% and 25% respectively of fair haired people. Is this difference likely be hidden in samples of 1200 and 900 respectively from the two populations.
...
# 14 It is claimed that a random sample of 100 tyres with a mean life of 15269 kms is drawn from a population of tyres which has a mean life of 15200 kms and a standard deviation of 1248 kms. Test the validity of the claim at (i). 5% and (ii) 1% level of significance.
...
# 15 Certain pesticide is packed into bags by a machine. A random sample of 10 bags drawn and their contents are found to weigh (in kg.) as follows:
...
# 17 An IQ test was administered to 5 persons before and after they were trained. The results are given below:
...
# 18 The number of scooter accidents per month in a certain town were as follows:
...
# 19 In an experiment on the immunization of humans from Covid 19, the following results were obtained. Find the expected frequency from this cross table.
...
# 22 The following table show the distribution of digits in number chosen at random from a telephone directory.
...
# 24 The mean return of two stocks in NSE-50 and their risk (variability) are as given below.Examine whether the difference in the variability in risk is significant at 5% level of significance.
